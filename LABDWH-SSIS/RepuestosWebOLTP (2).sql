...
if (((select COUNT(1) from sys.databases where name = 'RepuestosWeb')>0) AND (@EliminarDB = 1))
begin
	EXEC msdb.dbo.sp_delete_database_backuphistory @database_name = N'RepuestosWeb'
	
	
	use [master];
	ALTER DATABASE [RepuestosWeb] SET  SINGLE_USER WITH ROLLBACK IMMEDIATE;
		
	DROP DATABASE [RepuestosWeb]
	print 'RepuestosWeb ha sido eliminada'
end
...
INSERT INTO Linea (NombreLinea,DescripcionLinea) values ('Usados','Repuestos usados')
GO
...
INSERT INTO Categoria (ID_Linea,NombreCategoria,DescripcionCategoria) VALUES (@@IDENTITY,'Llantas','Llantas de vehiculos')
GO
...
INSERT INTO Partes (ID_Categoria,NombrePartes,DescripcionPartes,Precio) values(@@IDENTITY,'Llantas todo terreno','Llantas para caminos todo terreno',100.00)
go
